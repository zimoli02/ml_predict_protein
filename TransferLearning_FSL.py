...
def train_mlp(name, model, train_data, test_data, params):
    ...
    for epoch in range(params['n_epochs']):
        ...
        }, f'Models/FSL_Transferred_{name}.pth')
...
def main():
    for name in ['kuo_fepb', 'kuo_arti', 'kuo_dmsc','bonde', 'kosuri']:
        print(name)
        ...
        checkpoint = torch.load('Models/MAML_MetaTrained_Backbone.pth', map_location=torch.device('cpu'), weights_only=False)
        ...
        plt.tight_layout()
        fig1.savefig('Images/FSL_' + name + '_R2_Pre_Transfer.png')
        ...
        checkpoint = torch.load(f'Models/FSL_Transferred_{name}.pth', map_location=torch.device('cpu'), weights_only=False)
        ...
        axs.spines['right'].set_visible(False)
        plt.tight_layout()
        fig2.savefig('Images/FSL_' + name + '_MSE_Transfer_Learning.png')
        ...
        axs.scatter(y_test, pred_test, color = 'blue', alpha = 0.7)
        axs.scatter([], [], color = 'blue', label = "Post-Transferred R2: " + str(round(r2_score(y_test, pred_test),2)))
        start = min(min(y_test), min(pred_test))
        end = max(max(y_test), max(pred_test))
        line = np.arange(start, end, 0.01)
        axs.plot(line, line, color = 'black')
        ...
        axs.spines['right'].set_visible(False)
        plt.tight_layout()
        fig3.savefig('Images/FSL_' + name + '_R2_Post_Transfer.png')
